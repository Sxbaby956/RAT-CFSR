...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--no-constellation-stats",
        action="store_false",
        dest="use_constellation_stats",
        help="Disable the constellation-statistics semantic branch.",
    )
    parser.set_defaults(use_constellation_stats=True)
...
def make_model(args: argparse.Namespace, num_classes: int) -> RATCFSR:
    ...
    return RATCFSR(
        ...
        use_constellation_stats=args.use_constellation_stats,
    )
...
def run_test_only(args: argparse.Namespace) -> dict[str, object]:
    ...
    model = RATCFSR(
        ...
        use_constellation_stats=bool(model_config.get("use_constellation_stats", False)),
    ).to(device)
...
def run(args: argparse.Namespace, evaluate_after_training: bool = True) -> dict[str, object]:
    ...
    print(
        "dry_batch:",
        {
            ...
            "constellation": tuple(dry_outputs["constellation_semantic"].shape),
    ...
    backbone_parameters += list(model.constellation_encoder.parameters())
    backbone_parameters += list(model.fusion.parameters())
    backbone_parameters += list(model.constellation_gate.parameters())
    backbone_parameters += list(model.semantic_refine.parameters())
