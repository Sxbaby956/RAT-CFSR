...
def _open_batch_log_stream():
    if sys.stdout.isatty():
        return sys.stdout, False
    try:
        return open("/dev/tty", "w", buffering=1, encoding="utf-8"), True
    except OSError:
        return None, False
...
def train_epoch(
    ...
    batch_log_stream, should_close_batch_log = (
        _open_batch_log_stream() if log_interval > 0 else (None, False)
    )
    try:
        for batch_index, batch in enumerate(loader, start=1):
            ...
            if batch_log_stream is not None and (
                batch_index == 1
                ...
                print(
                    ...
                    file=batch_log_stream,
                    flush=True,
                )
    finally:
        if should_close_batch_log and batch_log_stream is not None:
...
def collect_outputs(
    ...
    return (
        ...
        np.concatenate(all_open_logits),
...
def open_head_unknown_scores(
    open_logits: np.ndarray, candidate_labels: np.ndarray
) -> np.ndarray:
    open_logits = np.asarray(open_logits, dtype=np.float64)
    candidate_labels = np.asarray(candidate_labels, dtype=np.int64)
    if open_logits.ndim != 2:
        raise ValueError("Expected open_logits [N, K]")
    if candidate_labels.shape != (open_logits.shape[0],):
        raise ValueError("Expected candidate_labels [N]")
    candidate_logits = open_logits[np.arange(open_logits.shape[0]), candidate_labels]
    return -candidate_logits


def correctly_classified_calibration_mask(
    calibration_logits: np.ndarray, calibration_labels: np.ndarray
) -> np.ndarray:
    calibration_logits = np.asarray(calibration_logits)
    calibration_labels = np.asarray(calibration_labels, dtype=np.int64)
    if calibration_logits.ndim != 2 or calibration_labels.shape != (
        calibration_logits.shape[0],
    ):
        raise ValueError("Expected calibration logits [N, K] and labels [N]")
    candidates = np.argmax(calibration_logits, axis=1)
    mask = candidates == calibration_labels
    class_ids = np.unique(calibration_labels)
    has_all_classes = all(
        np.sum(mask & (calibration_labels == class_id)) >= 2
...
def calibrate_and_predict(
    ...
    calibration_mask = correctly_classified_calibration_mask(
    ...
    if args.open_set_score == "prototype":
        ...
        prototypes = fit_feature_prototypes(
            calibration_features[calibration_mask],
            calibration_labels[calibration_mask],
        )
        ...
        calibrator = ClassConditionalScoreCalibrator(args.threshold_quantile).fit(
            calibration_scores[calibration_mask],
            calibration_labels[calibration_mask],
        )
        predictions = calibrator.predict(test_unknown_scores, candidate_labels)
        return calibrator, predictions, test_unknown_scores

    if args.open_set_score == "open_head":
        calibration_candidates = np.argmax(calibration_logits, axis=1)
        calibration_scores = open_head_unknown_scores(
            calibration_open_logits, calibration_candidates
        )
        test_unknown_scores = open_head_unknown_scores(
            test_open_logits, candidate_labels
        )
        calibrator = ClassConditionalScoreCalibrator(args.threshold_quantile).fit(
            calibration_scores[calibration_mask],
            calibration_labels[calibration_mask],
    ...
    calibrator = ClassConditionalScoreCalibrator(args.threshold_quantile).fit(
        calibration_scores[calibration_mask], calibration_labels[calibration_mask]
    )
    predictions = calibrator.predict(test_unknown_scores, candidate_labels)
    return calibrator, predictions, test_unknown_scores
...
def _early_stop_score(row: dict[str, object], metric: str) -> float:
    ...
    if metric == "val_acc":
        score = float(row["calibration_accuracy"])
        if int(row.get("stage", 0)) == 2:
            score -= 0.05 * float(row["val_loss"])
        return score
    raise ValueError(f"Unsupported early-stop metric: {metric}")


def _row_metric_value(row: dict[str, object], metric: str) -> float:
    if metric == "val_loss":
        return float(row["val_loss"])
    if metric == "val_acc":
...
def run(args: argparse.Namespace, evaluate_after_training: bool = True) -> dict[str, object]:
    ...
    best_stage1_epoch = 0
    best_stage1_row: dict[str, object] | None = None
    stage1_bad_epochs = 0
    ...
    if best_stage1_row is not None:
        print(
            "[status] "
            f"Stage 1 done: best_epoch={best_stage1_epoch} "
            f"best_{args.early_stop_metric}={_row_metric_value(best_stage1_row, args.early_stop_metric):.4f} "
            f"val_acc={best_stage1_row['calibration_accuracy']:.4f} "
            f"val_loss={best_stage1_row['val_loss']:.4f}; "
            "restored best warmup checkpoint"
        )
    else:
    ...
    class_parameters = (
        ...
        + list(model.open_heads.parameters())
    )
    ...
    if best_stage2_row is not None:
        print(
            "[status] "
            f"Stage 2 done: best_epoch={best_stage2_epoch} "
            f"best_{args.early_stop_metric}={_row_metric_value(best_stage2_row, args.early_stop_metric):.4f} "
            f"val_acc={best_stage2_row['calibration_accuracy']:.4f} "
            f"val_loss={best_stage2_row['val_loss']:.4f}; "
            "restored best fine-tuned checkpoint"
        )
    else:
